...
export const updateAdjusterStatus = (id, status) => api.put(`/adjusters/${id}/status`, { status });
...
//   };
  ...
  export const getClientsWithAdjusters = () => {
    return fetch('http://localhost:5000/api/clients/with-adjusters')
...
export const assignAdjusterToClient = (clientId, adjusterId) => {
  ...
  export const getClientsForAdjuster = async () => {
    const response = await fetch('http://localhost:5000/api/adjusters/me/clients', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
